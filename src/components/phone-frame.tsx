...
export function PhoneFrame({ className, children, ...props }: PhoneFrameProps) {
  return (
    <div
      {...props}
      className={cn(
        "relative w-72 shrink-0 rounded-3xl border border-border-strong bg-background-sunken p-2 shadow-raised",
        className,
      )}
    >
      {/* aspect-[9/19.5] is a hardware fact (modern phone screen), not a
          design value; no token category exists for aspect ratios. */}
      <div className="relative isolate aspect-[9/19.5] overflow-hidden rounded-2xl bg-background">
        <div
          aria-hidden
